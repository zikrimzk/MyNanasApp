...
class EntrepreneurAddProductFragment : Fragment() {
    ...
    private val categoryList = listOf("Fresh Pineapple", "Processed Goods", "Seeds/Slips", "Fertilizer", "Tools")
    // Expanded Unit List
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        btnSave.setOnClickListener {
            if (actvPremise.text.isEmpty()) {
                Toast.makeText(context, "Please select a premise", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
            if (etName.text.isNullOrBlank() || etPrice.text.isNullOrBlank()) {
                Toast.makeText(context, "Please fill in required fields", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }

            Toast.makeText(context, "Product submitted for review!", Toast.LENGTH_LONG).show()
